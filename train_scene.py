...
def main(argv=None):
    ...
    def train(epoch):
        ...
        for i, (features, adjs, gmms, evaluates,scene_labels,zero_nodes,zero_masks) in enumerate(train_loader):
            ...
            loss_train = loss_fn(outputs, scene_labels) # 计算一个batch的loss
            loss_train.backward()
            optimizer.step()
            loss_train_epoch = loss_train_epoch + loss_train.data.item()

        loss_train_epoch = loss_train_epoch/len(train_loader)#计算出train_loss平均值
        ...
        if not args.fastmode:
            ...
            with torch.no_grad():
                for i, (features, adjs, gmms, evaluates,scene_labels,zero_nodes,zero_masks) in enumerate(val_loader):
                    ...
                    outputs = model(features, Local_f, adjs)
                    outputs = torch.reshape(outputs, (-1, args.class_num))
                    loss_val = loss_fn(outputs, scene_labels)
    ...
    copyfile(os.getcwd() + '/' + str(best_epoch)+'.pkl', os.getcwd() + '/model/3D-SPAN-B.pkl')
